...
echo "Log: $LOG"
echo ""
echo "To use with Claude Code (TabbyAPI is OpenAI-only — needs a LiteLLM proxy):"
echo "  1. Run LiteLLM mapping a model_name -> openai/<tabby-model> at :4000 (see README Cursor section)"
echo "  2. ANTHROPIC_BASE_URL=http://localhost:4000 ANTHROPIC_AUTH_TOKEN=local claude --model qwen"
echo "  (No proxy? Use Ollama or vllm-mlx instead — those serve the native Anthropic API.)"
echo ""
